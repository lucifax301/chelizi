...
app.controller("Student",["$scope","$filter",function($s,$filter){
	...
	function getJson(nowPage){
		var json={
			url:config.basePath+"vipcoach/batch",
			data: {
				"pageNo": nowPage,
...
/*------------------------------------------编辑 | 新增 教练车---------------------------------------------*/
	...
	$s.editData={};
	$s.editType="add";
	$s.vipCoachEdit=function(type,data){
	...
	$s.submitEditMsg=function($event){
		var json=$s.editType=="add"?editVipCoachJson("vipcoach/add"):editVipCoachJson("vipcoach/update");

		if(!$s.editData.name || !regCombination('*').test($s.editData.name)){
			Layer.alert({type:"msg",title:"请填写大客户教练名称"});
			return false;	
		}
