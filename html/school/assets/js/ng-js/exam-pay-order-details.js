...
/*--------------------------------------订单详情-----------------------------------------------------*/
	...
	$s.getOrderDetailsData=function(){
		$.AJAX({
			type:"get",
			url:config.basePath+"examPlace/payorder/info",
			data:{
				orderId : $s.orderId
			},
			success:function(data){
				...
				//console.log($s.data)
				var tmpinfo=$s.data.orders;
				var clssinfo="";
				for(var i=0;i<tmpinfo.length;i++){
					//console.log(tmpinfo[i])
					clssinfo=clssinfo+tmpinfo[i].carNo+" "+$filter("timeTex")(tmpinfo[i].pstart)+"~"+$filter("timeTex")(tmpinfo[i].pend)+"\r\n";
				}
				$s.data.orderinfo=clssinfo;
                console.log($s.data)
				$s.$apply();
			}
		});
