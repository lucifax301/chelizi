...
app.controller("ExamGroundOrder",["$scope","$filter",function($s,$filter){
    ...
    $s.placeId="";//考场id。查询全部则传空。
    $s.coachName="";//模糊搜索：教练姓名
    $s.schoolName = "";//模糊搜索：驾校名称
    $s.carNo="";//模糊搜索：车牌号
    $s.coachMobile="";//模糊搜索：教练手机号
    $s.orderId="";//模糊搜索：订单号
    $s.searchType = "orderId";//默认搜索类型，订单号
    $s.search = "";
    ...
    /*按订单状态筛选列表数据*/
    $s.getDataForOrderState=function($event,state){
        $($event.target).addClass('active').siblings().removeClass("active");
        win.showLoading();
        $s.defaultPage=1; //默认第一页
        $s.state=state;
    ...
    /*按练考车型筛选列表数据*/
    $s.getDataForCarType=function($event,state){
        $($event.target).addClass('active').siblings().removeClass("active");
        win.showLoading();
        $s.defaultPage=1; //默认第一页
        $s.drtype=state;
    ...
    /*-----------------------------------关闭订单------------------------------------------------------*/
    /*关闭订单*/
    $s.closeOrder=function(itemData){
        //Layer.confirm({width:300,height:160,title:"确认关闭该订单吗？",header:"关闭订单"},function(){
        //    $.AJAX({
