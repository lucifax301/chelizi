...
PopLayer.prototype.confirmNotByTextAlert=function(json,fn){
	...
	<div class='custom-msg-but'>\
                    <div class='margin-bottom-15'>"+json.title+"</div>\
                    <textarea class='form-control' id='withdraw-NotBy' rows='3' placeholder='"+botByText+"' oninput='changButton()'></textarea>\
	...
	/**-------------新增textNotNull控制理由必填------------开始-------------*/
	if(json.textNotNull==true){
		$(".btn-notBy-submit").attr("disabled",true);
	}
	window.changButton=function(){
		var notByText=document.getElementById('withdraw-NotBy').value;
		if(json.textNotNull==true && notByText==""){
			$(".btn-notBy-submit").attr("disabled",true);
		}else{
			$(".btn-notBy-submit").attr("disabled",false);
		}
	}
	/**--------------------新增textNotNull控制理由必填---------结束------------*/
