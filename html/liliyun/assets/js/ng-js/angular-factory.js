...
/*
 暴露的接口：
    ...
    getMsgTime.shotMessage(function(){
		//一个回调函数时间走完后执行
    });    定时器开始执行
*/
app.factory('getMsgTime', ['$rootScope', function($s){
    return {
        getNowTime:function(){
           $s.isClick=localStorage.getItem("nowMsgTime")<config.msgTime?false:true;
           $s.getMsgText="获取验证码";
           //运行函数
           $s.runTime=function(fn){
           		if(localStorage.getItem("nowTime")){
                var timer=setInterval(function(){
	                var nowTime=config.msgTime-Math.floor((new Date().getTime()-localStorage.getItem("nowTime"))/1000);
	                $s.nowMsgTime=nowTime;
	                localStorage.setItem("nowMsgTime",nowTime);
	                $s.$apply();
	                 //时间走完时执行
	                if(nowTime<=0){
	                    clearInterval(timer);
	                    $s.isClick=true; 
	                    localStorage.setItem("nowMsgTime",config.msgTime);
	                    $s.nowMsgTime="";
	                    $s.getMsgText="重新获取";
	                    if(fn){fn()};
	                    $s.$apply();  
           ...
           if(localStorage.getItem("nowMsgTime")<config.msgTime){
                $s.runTime();
           };
        },
        shotMessage:function(fn){
        	$s.getMsgText="重新获取";
        	//设置初始总时间
            localStorage.setItem("nowMsgTime",config.msgTime);
            //设置当然前时间
