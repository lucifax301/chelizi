...
/*main 控制器*/
app.controller("ImportSchool",["$scope","$rootScope","$filter","Selects",function($s,$rootScope,$filter,Selects){
	...
	$s.submitCoachMsg=function($event){
		if(!regCombination('*').test($s.addCoachNub)){
			Layer.alert({width:300,height:160,type:"msg",title:"请输入追加发行的数量!",header:"追加发行"});
			return false;
		}
		if(($s.addCoachNub <= 0) ||(isNaN($s.addCoachNub))){
			Layer.alert({width:300,height:160,type:"msg",title:"追加发行的数量不合法!",header:"追加发行"});
			return false;
		}
		$.AJAX({
	...
	$s.activeAction=function(data){
		if(data.isexist==0){  //判断该优惠券是否生效  0-未生效  1-生效
			Layer.alert({width:335,height:175,type:"msg",title:"该优惠券未生效，请选择已生效<br>的优惠券模板！",header:"停止发放"});
			return false;
		}
		if(parseInt(data.verify)!=1){
			Layer.alert({width:300,height:160,type:"msg",title:"该优惠券未经审核，不能停发!",header:"追加发行"});
			return false;
		}
		Layer.confirm({width:300,height:160,title:"确认停用选中券模板吗？",header:"停止发放"},function(){
			$.AJAX({
	...
	$s.searchCouponDetails=function(phoneNum){
		...
		$.AJAX({
			...
			success:function(data){
				$s.bindCouponDetails=JSON.parse(data.result.pageData).dataList[0];
				$s.$apply();
			}
		});
	}
    ...
    $s.writtenOff=function(){
		...
		var istype=true,studentid=[],coupontmpid=[];   //isused使用状态    studentid学员id   coupontmpid优惠券id
		for (var i=0;i<datas.length;i++){
			studentid.push(datas[i].studentid);
			coupontmpid.push(datas[i].coupontmpid);
			if (datas[i].type != 1) {
				istype=false;
				break;
			}
		...
		Layer.confirm({width:315,height:170,title:"注销后不可恢复，您确认注销<br>所选优惠券吗？",header:"注销"},function(){
			$.AJAX({
				...
				data:{
					couponTmpId:coupontmpid.toString(),
