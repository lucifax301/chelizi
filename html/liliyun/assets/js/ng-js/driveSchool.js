...
/*main 控制器*/
app.controller("DriveSchool",["$scope","$filter",function($s,$filter,$scope){
...
/*------------------------------------------编辑 | 新增 驾校---------------------------------------------*/
	...
	$s.editData={};
	$s.editType="add";
	$s.driveSchoolEdit=function(type,data){
	...
	function editSchoolJson(url){
		var json={
				...
				cityId:parseInt($s.editData.cityId),
				 score:$s.editData.score,
			     logo:$s.editData.logo,
				 icon:$s.editData.icon,
				 image:$s.editData.image,
			     lge:$s.editData.lge,
				 lae:$s.editData.lae,
				 schoolInfo:$s.editData.schoolInfo,
				 regInfo:$s.editData.regInfo,
		     	tell:$s.editData.tell,
		     	address:$s.editData.address,
		    	storeCount:$s.editData.storeCount,
			coachCount:$s.editData.coachCount,
			carCount:$s.editData.carCount,
			trfieldCount:$s.editData.trfieldCount
			};
	...
	$s.submitEditMsg=function($event){
		...
		var json=$s.editType=="add"?editSchoolJson("school/addWxSchool"):editSchoolJson("school/updateWxSchool");
		...
		if(!$s.editData.score){
			Layer.alert({width:300,height:150,type:"msg",title:"请填写分数"});
			return false;
		}
		if(!regCombination('number').test($s.editData.score)){
			Layer.alert({width:300,height:150,type:"msg",title:"分数请填写正整数"});
			return false;
		}
		...
		if(!regCombination('tell').test($s.editData.tell)){
			Layer.alert({width:300,height:150,type:"msg",title:"请填写正确的电话"});
			return false;
		}
		if(!$s.editData.address){
			Layer.alert({width:300,height:150,type:"msg",title:"请填写总部地址"});
			return false;
		}


	//	checkDataList({name:$s.editData.name},"school/queryDriveSchool");
