var app=angular.module("app",[]) //["ngFilter","ngSelects"]  依赖于其他模块
/*main 控制器*/
app.controller('RoleAdd',["$scope",function($s){
    ...
    $s.getAllMenus = function(){
        $.AJAX({
            type:"get",
            data:{id:$s.roleId},
            url:config.basePath+"privilege/getRole",
            success:function(data){
                $s.permissionData = data.data.privileges;
                $s.permissionName = data.data.name;
                $s.$apply();
            }
        })
    }
    ...
    $s.submitData = function(){
        ...
        Layer.confirm({width:320,height:160,title:"确认修改该角色？",header:"编辑角色"},function() {
            var submitStr = submitArr.toString();
            $.AJAX({
                url:config.basePath+"privilege/updateRole",
                data:{
                    id:$s.roleId,
                ...
                success:function(data){
                    ...
                    setTimeout(function(){window.location.href="user-permission.html"},1500)
                }
            })
        })
