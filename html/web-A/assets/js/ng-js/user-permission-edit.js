...
/*main 控制器*/
app.controller('RoleEdit',["$scope",function($s){
    $s.roleId = getUrl('roleId');
    ...
    $s.getAllMenus = function(){
        $.AJAX({
            ...
            success:function(data) {
                $s.permissionData = data.result.privileges;
                $s.name = data.result.name;
                $s.remark = data.result.remark;
                $s.$apply();
            }
        })
    }
