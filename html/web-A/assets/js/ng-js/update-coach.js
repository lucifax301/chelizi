...
app.controller("updateCoach",["$scope","$filter","$http",function($scope,$filter,$http){
	...
	$.AJAX({
            url : config.basePath + "coach/getCoach",
            type : "POST",
			data:{coachid:coachid},
            success : function(data) {
				$scope.editData = data.result;
				$("#fstdrilicdate").val($scope.editData.fstdrilicdate); 
				$("#teachStartDate").val($scope.editData.teachStartDate); 
				$("#hiredate").val($scope.editData.hiredate); 
				$("#leavedate").val($scope.editData.leavedate); 
				
				var photo_url = $scope.baseImgurl + $scope.editData.photo_url;
				if($scope.editData.photo_url){
				$('#headImage').attr("src", photo_url) ;
				}
				$scope.$apply();
