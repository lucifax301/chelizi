...
public class WxSignupPurpose implements IPayPurpose  {
	 @Autowired
...
//	        }
	        ...
	        // 记录资金流水
	           moneyManager.handleWxSignupMoneyFlow(payVo);

	        try {
				// 修改报名费支付状态：0代表未支付，1代表已支付,2代表支付失败，9代表老学员自动支付'
				schoolManager.postWxPayState(payVo, ReqConstants.STAGE_STATE_SUCC);
			} catch (Exception e) {
				e.printStackTrace();
			}
	    ...
	    public Object purposeAdvance(PayVo payVo, ReqResult reqResult) {
	        ...
	        //如果已经付过款
	        int payState = schoolManager.getWxPayState(payVo.getPayOrderId());
