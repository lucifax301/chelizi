...
public class ScanPayBusiness {
    ...
    public interface ResultListener {
        ...
        void onFailByReturnCodeError(PayResData scanPayResData);

        //API返回ReturnCode为FAIL，支付API系统返回失败，请检测Post给API的数据是否规范合法
        void onFailByReturnCodeFail(PayResData scanPayResData);

        //支付请求API返回的数据签名验证失败，有可能数据被篡改了
        void onFailBySignInvalid(PayResData scanPayResData);
        ...
        void onFailByAuthCodeExpire(PayResData scanPayResData);

        //授权码无效，提示用户刷新一维码/二维码，之后重新扫码支付"
        void onFailByAuthCodeInvalid(PayResData scanPayResData);

        //用户余额不足，换其他卡支付或是用现金支付
        void onFailByMoneyNotEnough(PayResData scanPayResData);

        //支付失败
        void onFail(PayResData scanPayResData);

        //支付成功
        void onSuccess(PayResData scanPayResData,String transactionID);
     ...
     * @throws Exception
     */
    public void run(ScanPayReqData scanPayReqData, String key, String mchId, ResultListener resultListener) throws Exception {
        ...
        ReportReqData reportReqData = new ReportReqData(
                ...
                scanPayReqData.getSpbill_create_ip(), scanPayReqData.getAppid(), key, mchId
        );
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            if (scanPayResData.getResult_code().equals("SUCCESS")) {
                ...
                if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("AUTH_CODE_INVALID") || errorCode.equals("NOTENOUGH")) {
                    ...
                    //对于扣款明确失败的情况直接走撤销逻辑
                    doReverseLoop(outTradeNo, scanPayReqData.getAppid(), key, mchId, resultListener);
                    ...
                    if (doPayQueryLoop(payQueryLoopInvokedCount, outTradeNo, scanPayReqData.getAppid(), key, mchId, resultListener)) {
