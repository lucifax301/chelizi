...
public class DeleteCacheInterceptor {
	private Logger log=Logger.getLogger(DeleteCacheInterceptor.class);
	@Autowired
	...
	public Object doAround(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			redisRealTime.delete(joinPoint.getArgs()[0],
					(BaseService) joinPoint.getThis());
			// 缓存失败不能抛异常
		} catch (Exception e) {
			log.error(
					joinPoint.getArgs()[0] + " cache Exception:"
							+ e.getMessage(), e);
		}
		return object;
	}
