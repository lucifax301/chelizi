...
public class DepositServiceImpl implements ICmsDepositService {
	Logger logger = Logger.getLogger(DepositServiceImpl.class);
	...
	public String query(DepositVo depositVo) {
		String resp = null;
		try {
			logger.info("************************* depositVo " + depositVo.getCheckStatus());
			PagedResult<DepositVo> batchDeposit = depositManager.queryBatchDepositList(depositVo);
	...
	public ResponseMessage pass(String checker, String checkRemark, String id) {
		try {
			...
			List<DepositVo> depositList = depositManager.queryDepositInfo(ids); //批量查询
			...
			depositManager.batchUpdateStatusList(updateDepositList);  //批量更新状态
	...
	public ResponseMessage reject(String checker, String checkRemark, String id, String handlerType) {
		...
		try {
			...
			depositManager.batchUpdateStatusList(updateDepositList);  //批量更新提现表状态
			logger.info("---------------------------------------DepositController  Update User Status Fail Success!");
			 
			int money;
			for(int i = 0; i < depositList.size(); i++){
				Integer depMoeny  = Math.abs(depositList.get(i).getMoney());
				if(Constant.COACH_USER_TYPE.equals(depositList.get(i).getUserType())) {   //教练
					//查金额
					CoachAccount coachAccount = coachAccountService.getCoachIdMoney(depositList.get(i).getUserId());
					money  = coachAccount.getMoney() + depMoeny;  
					coachAccountService.addMoneyBack(money, depositList.get(i).getUserId());//回退预扣金额
					logger.info("---------------------------------------DepositController CoachId: " + depositList.get(i).getUserId() + ", Old Money: "+ 
							coachAccount.getMoney() + ", Add Money : " + depositList.get(i).getMoney() + ", Success!");
					//记录日志
					addMoneyLog(depositList.get(i).getUserId(), depositList.get(i).getUserType(), money, 
							depMoeny,  depositList.get(i).getOrderId());
				}
				else if (Constant.STUDENT_USER_TYPE.equals(depositList.get(i).getUserType())) { //	学员
					StudentAccount studentAccount = studentAccountService.getStudentIdMoney(depositList.get(i).getUserId());
					money  =  studentAccount.getMoney()+ depMoeny; 
					studentAccountService.addMoneyBack(money, depositList.get(i).getUserId());//回退预扣金额
					logger.info("---------------------------------------DepositController Student Id: " + depositList.get(i).getUserId() + ", Old Money: "+ 
							studentAccount.getMoney() + ", Add Money : " + depositList.get(i).getMoney() + ",Success!");
					//记录日志
					addMoneyLog(depositList.get(i).getUserId(), depositList.get(i).getUserType(), money, 
							depMoeny,   depositList.get(i).getOrderId());
				}
    ...
    public ResponseMessage tellerTransfer(String transfer, String id) {
		int errorId = 0;
		DepositVo backDeposit = new DepositVo();
		...
		try {
			...
			depositManager.tellerTransferUpdateStatus(updateDepositList);  //批量更新状态
			logger.info("---------------------------------------DepositController  Update  Status To Success Finash!");
			...
			if ( errorId != 0){
				...
				backDeposit.setMerDate(DateUtil.getTodayDate());
				backDeposit.setMerSeqId(merSeqId);
				depositManager.updateStatus(backDeposit);;  //更新状态异常
			}
			return new ResponseMessage(MessageCode.MSG_FAIL);
		}
    }
	...
	public Integer queryTotalMoney(DepositVo depositVo) {
		return  depositManager.queryCountMoneyLimitList(depositVo);
	}

	@Override
	public List<DepositVo> queryDownDepositList(DepositVo depositVo) {
		return depositManager.downLoadDepositList(depositVo);
	}
